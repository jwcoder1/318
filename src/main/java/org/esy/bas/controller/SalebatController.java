...
import org.esy.bas.service.ISalebatService;
 ...
 */
@Controller
@RequestMapping("/api/bas/salebat")
public class SalebatController {

    public static final String AUTHORITY = "bas_salebat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ISalebatService salebatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Salebat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", salebatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Salebat o = salebatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Salebat, pageable
	 * @return HttpResult
	 * @date Wed Jun 24 13:15:44 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Salebat salebat, Pageable pageable) {
		if (salebat == null) {
			salebat = new Salebat();
		}
		return new HttpResult(salebatService.query(salebat, pageable));
	}
