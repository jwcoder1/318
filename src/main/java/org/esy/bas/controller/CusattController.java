...
import org.esy.bas.service.ICusattService;
 ...
 */
@Controller
@RequestMapping("/api/bas/cusatt")
public class CusattController {

    public static final String AUTHORITY = "bas_cusatt";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ICusattService cusattService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Cusatt o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", cusattService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Cusatt o = cusattService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Cusatt, pageable
	 * @return HttpResult
	 * @date Tue Jun 23 13:30:12 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Cusatt cusatt, Pageable pageable) {
		if (cusatt == null) {
			cusatt = new Cusatt();
		}
		return new HttpResult(cusattService.query(cusatt, pageable));
	}
