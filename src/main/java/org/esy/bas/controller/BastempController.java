 ...
 */
@Controller
@RequestMapping("/api/bas/bastemp")
public class BastempController {

    public static final String AUTHORITY = "bas_bastemp";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBastempService bastempService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Bastemp o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", bastempService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Bastemp o = bastempService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Bastemp, pageable
	 * @return HttpResult
	 * @date Fri Jun 19 11:53:17 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Bastemp bastemp, Pageable pageable) {
		if (bastemp == null) {
			bastemp = new Bastemp();
		}
		return new HttpResult(bastempService.query(bastemp, pageable));
	}
