...
public class AccbatController {
	...
	@Autowired
	private IAccbatService accbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Accbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", accbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Accbat o = accbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Accbat, pageable
	 * @return HttpResult
	 * @date Sat Jun 20 09:30:48 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Accbat accbat, Pageable pageable) {
		if (accbat == null) {
			accbat = new Accbat();
		}
		return new HttpResult(accbatService.query(accbat, pageable));
	}
