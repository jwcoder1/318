 ...
 */
@Controller
@RequestMapping("/api/acr/aprnbat")
public class AprnbatController {

    public static final String AUTHORITY = "acr_aprnbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IAprnbatService aprnbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Aprnbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", aprnbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Aprnbat o = aprnbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Aprnbat, pageable
	 * @return HttpResult
	 * @date Tue Jun 23 11:02:07 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Aprnbat aprnbat, Pageable pageable) {
		if (aprnbat == null) {
			aprnbat = new Aprnbat();
		}
		return new HttpResult(aprnbatService.query(aprnbat, pageable));
	}
