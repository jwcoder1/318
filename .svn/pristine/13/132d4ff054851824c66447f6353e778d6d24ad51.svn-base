 ...
 */
@Controller
@RequestMapping("/api/bas/workbat")
public class WorkbatController {

    public static final String AUTHORITY = "bas_workbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IWorkbatService workbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Workbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", workbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Workbat o = workbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Workbat, pageable
	 * @return HttpResult
	 * @date Fri Jun 19 15:10:39 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Workbat workbat, Pageable pageable) {
		if (workbat == null) {
			workbat = new Workbat();
		}
		return new HttpResult(workbatService.query(workbat, pageable));
	}
