 ...
 */
@Controller
@RequestMapping("/api/acr/acrdis")
public class AcrdisController {

    public static final String AUTHORITY = "acr_acrdis";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IAcrdisService acrdisService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Acrdis o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", acrdisService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Acrdis o = acrdisService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Acrdis, pageable
	 * @return HttpResult
	 * @date Tue Jun 23 11:01:13 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Acrdis acrdis, Pageable pageable) {
		if (acrdis == null) {
			acrdis = new Acrdis();
		}
		return new HttpResult(acrdisService.query(acrdis, pageable));
	}
