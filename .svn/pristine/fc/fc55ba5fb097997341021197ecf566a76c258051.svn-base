...
public class AprnbahController {
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Aprnbah o, HttpServletRequest request) {
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Aprnbah o = aprnbahService.getByUid(uid);
		if (o == null) {
	...
	public HttpResult query(@Valid @RequestBody(required = false) Aprnbah aprnbah, Pageable pageable) {
		if (aprnbah == null) {
			aprnbah = new Aprnbah();
		}
